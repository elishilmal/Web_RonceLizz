...
try {
    ...
    $stmt = $pdo->prepare("SELECT py.*, o.order_code, o.quantity, o.total_price, p.name as product_name,
        ...
        try {
            ...
            try {
                ...
                if ($isCOD) {
                    ...
                    $stmt = $pdo->prepare("UPDATE orders SET status = 'cod_confirmed' WHERE id = ? AND user_id = ?");
                    $stmt->execute([$order_id, $_SESSION['user_id']]);
                } else {
                    // Untuk non-COD, status order tetap 'pending' menunggu verifikasi
                    $stmt = $pdo->prepare("UPDATE orders SET status = 'payment_pending' WHERE id = ? AND user_id = ?");
                    $stmt->execute([$order_id, $_SESSION['user_id']]);
                }
                ...
                if ($isCOD) {
                    $message = "Pesanan COD berhasil dibuat! Status pesanan telah berubah menjadi 'COD Dikonfirmasi'. Tunggu admin memproses pesanan Anda.";
                } else {
                    $message = "Bukti pembayaran berhasil diupload! Status pesanan telah berubah menjadi 'Menunggu Verifikasi'. Admin akan memverifikasi pembayaran Anda.";
                }
